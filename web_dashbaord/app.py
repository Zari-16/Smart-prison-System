...
app.config["MAIL_SERVER"] = os.getenv("MAIL_SERVER", "fairyq801@gmail.com")
...
INFLUX_CONTROL_TOKEN = os.getenv("INFLUX_CONTROL_TOKEN", "qjY1cFVn4YcnpI3nmYZT454K9HHjDFaEKJ3ftgQqbg3sCkZcHKv8dD7ZHGhiOxm4cmYQ4viEEbNqQ_l8yiv5OA==")
INFLUX_PATROL_TOKEN = os.getenv("INFLUX_PATROL_TOKEN", "_OTXDBpW35G-H70RjyKn2QHZAlwcSU7mETHX3vmn-7dnD1WAoXnC8jrzForcxTz7HJ6DZpQUUXn_alcPEYgE2Q==")
...
def init_db():
    with app.app_context():
        ...
        if not User.query.first():
            ...
            guard = User(
                username="guard",
                password_hash=generate_password_hash("guardpass"),
                role="level1",
                email=os.getenv("GUARD_EMAIL", "")
            )
...
def api_status():
    ...
    # Convert influx results into structured data
    for p in control_points:
        if p["field"] == "people_count":
            control_data["people_count"] = p["value"]
        elif p["field"] == "door_open":
            control_data["door_open"] = p["value"]
        elif p["field"] == "fence_alert":
            control_data["fence_alert"] = p["value"]

    for p in patrol_points:
        if p["field"] == "temperature":
            patrol_data["temperature"] = p["value"]
        elif p["field"] == "humidity":
            patrol_data["humidity"] = p["value"]

    return jsonify({
        "status": "success",
        "control_room": control_data,
        "sensors": patrol_data
    })
...
def get_sensor_data():
    ...
    from(bucket: "{INFLUX_BUCKET}")
      |> range(start: -1h)
      |> filter(fn: (r) => r._measurement == "prison_sensors")
      |> pivot(rowKey:["_time"], columnKey: ["_field"], valueColumn: "_value")
      |> sort(columns: ["_time"])
    '''

    client = get_influx_client(INFLUX_PATROL_TOKEN)
    if not client:
        return jsonify([])
    ...
    for table in result:
        for record in table.records:
            data.append({
                ...
                "gas": record.values.get("gas", 0),
                "water": record.values.get("water", 0),
                "motion": record.values.get("motion", 0),
                "alert_state": record.values.get("alert_state", 0)
            })
